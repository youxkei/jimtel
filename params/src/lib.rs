        ...
        Data::Struct(data) => match data.fields {
            Fields::Named(fields) => {
                let fields = fields
                    ...
                    .into_iter()
                    .enumerate()
                    .map(|(i, field)| (i as i32, field.ident.unwrap()))
                    .collect::<Vec<_>>();
                let num_fields = fields.len();

                let get_paramater_matches = fields
                    .iter()
                    .map(|(i, field)| quote! {#i => self.#field.get()});
                let set_paramater_matches = fields
                    .iter()
                    .map(|(i, field)| quote! {#i => self.#field.set(value)});
